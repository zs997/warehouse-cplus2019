...
/*********************************************************************************************************/
	...
	for(;iterator!=((*ans).end()-1);iterator++) //最后一个对象是总和概念不一样
	{	 
		dataFile<<(*iterator).rowNO<<"   "<<(*iterator).numA<<"   "<<(*iterator).numB
			<<"   "<<(*iterator).numC<<"   "<<(*iterator).value<<endl;
		sum+=(*iterator).value;
	}

	double average_routing_length=(*iterator).value;//最后一个对象
		dataFile<<(*iterator).rowNO<<"   "<<(*iterator).numA<<"   "<<(*iterator).numB
			<<"   "<<(*iterator).numC<<"   "<<average_routing_length<<endl; //最后一个对象，表示总和
		...
		if((sum-average_routing_length<0.01)&&(sum>0)&&(average_routing_length>0))
			dataFile<<"delta 1 合格"<<endl;
		else
			dataFile<<"delta 1 不合格"<<endl;
	dataFile<<"time_cost="<<time_cost<<endl;
	dataFile<<"caseNO="<<caseNO<<endl<<"S-shape"<<endl;
	dataFile.close();
	return 1;
