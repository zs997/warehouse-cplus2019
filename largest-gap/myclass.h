...
{
   	public: 
			static void calcul_pij(); //1，计算但是不知道怎么计算的		   
			static void calcul_pij(int flag);

		    static void calcul_overallABC(); //2
			static void calcul_overallABC(int flag); 
...
public:
	...
	void assignment_abc(int numA,int numB,int numC,int flag);
	...
	void do_ours(int numA,int numB,int numC,int flag);
