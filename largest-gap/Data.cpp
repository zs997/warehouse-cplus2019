...
/****************************************************************************************************************/
	...
	double average_routing_length=(*iterator).value;//最后一个对象
		...
		dataFile<<"time_cost="<<time_cost<<"s"<<endl<<"caseNO="<<caseNO<<endl<<"larget_gap"<<endl;
...
/******************读取解数据***********************************************/
	...
	int sumA=0,sumB=0,sumC=0,flag;
	
	culum cu_check;
	double firstprob_temp,enterprob_temp;
	int NO=1;
	while(!ve.empty())
	{
		flag=ve.back();ve.pop_back();
		ve.pop_back();
		...
		{
			cu_check.do_ours(numA,numB,numC,flag);
			cu_check.calcul_cost(NO,sumA,sumB,sumC,firstprob_temp,enterprob_temp);

		}
		
		firstprob_temp=cu_check.get_firstprob();
		enterprob_temp=cu_check.get_enterprob();
...
/***************************写入检验解数据，*************************************************/
	 ...
	 dataFile<<"检验解数据（根据解再次计算）结果"<<endl;
